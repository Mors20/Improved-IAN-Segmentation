...
class UNet3D(nn.Module):
    ...
    def forward(self,  query_points , x):
        implicit_features = []
        ...
        implicit_features = F.grid_sample(dec1, query_points.unsqueeze(2).unsqueeze(3).float(),padding_mode= 'border', align_corners=False)
        outputs = self.conv(implicit_features)
        return outputs
